...
Суть реализаций примерно одна и та же. Есть некоторое количество узлов в кластере, назовем его Workers, и есть список задач (задача = {Функция, Аргументы}). Система равномерно распределяет задачи из списка по узлам кластера, потом собирает и объединяет результаты. Для примера я сделал Map и Reduce на своих системах. Они автоматически разбитвают входные данные на группы, формируют из них список задач и отправляют на исполнение (а в случае Reduce повторяют до тех пор, пока не останется единственный элемент).

Пример обработки ошибок. Один из узлов не может выполнить свою функцию из-за неверных входных данных. Reduce в данном случае вернет только сообщение об ошибке, а Map сделается для всех правильных данных.
